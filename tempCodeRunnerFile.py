...
def draw_pause_screen():
    ...
    instruction1 = INSTRUCTION_FONT.render("ESC - Wznów grę", True, "white")
    instruction1_rect = instruction1.get_rect(center=(WIDTH//2, HEIGHT//2 + 20))
    WINDOW.blit(instruction1, instruction1_rect)
    
    instruction2 = INSTRUCTION_FONT.render("R - Restart gry", True, "white")
...
def draw(player, objects, enemies, paused=False):
    ...
    # Wyświetl liczbę żyć
    lives_text = FONT.render(f"Życia: {player.lives}",True,"white")
    WINDOW.blit(lives_text,(10,50))
    
    # Rysuj ekran pauzy jeśli gra jest zatrzymana
    if paused:
        draw_pause_screen()
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # Restart tylko podczas pauzy
                if event.key == pygame.K_r and GAME_PAUSED:
                    player, enemies = restart_game()
                    GAME_PAUSED = False
                
                # Skok tylko gdy gra nie jest zatrzymana
                if (event.key == pygame.K_w or event.key == pygame.K_SPACE) and not player.jump and not GAME_PAUSED:
                    player.do_jump()

        # Aktualizuj grę tylko gdy nie jest zatrzymana
        if not GAME_PAUSED:
            handle_move(player)
        ...
        draw(player, objects, enemies, GAME_PAUSED)
